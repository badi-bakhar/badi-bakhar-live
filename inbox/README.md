...
| File       | Purpose                                           | Touch? |
|------------|---------------------------------------------------|--------|
| `soul`     | Question capture queue — append new souls here    | Yes    |
...
Open `soul`, copy the template block at the top, fill it in, append it at the bottom.
...
1. **Capture** — drop it in `soul` with `status: open`. Takes 30 seconds.
2. **Triage** — once a week, scan all `status: open` entries. Promote urgency, add context, or archive junk.
3. **Process** — turn the soul into a note, a decision, an action, or an answer. Move it out of inbox.
...
Any soul sitting at `status: open` for more than 30 days without progress
...
It reads `soul`, surfaces patterns, suggests answers or next steps, and helps you
